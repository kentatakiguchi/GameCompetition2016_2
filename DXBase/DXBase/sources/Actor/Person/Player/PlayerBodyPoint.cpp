...
PlayerBodyPoint::PlayerBodyPoint(IWorld * world, const Vector2 & position, const int& index) :
	Actor(world, "BodyPoint" , position, std::make_shared<BoundingCircle>(Vector2::Zero, Matrix::Identity, PLAYER_RADIUS * 0.2f, true)), index_(index),
	stiffness_(4.0f),
	friction_(0.1f),
	mass_(0.8f) {
}
...
void PlayerBodyPoint::onCollide(Actor & other){
	if (other.getName() == "MovelessFloor") {
		...
		if (top > 0 && left <= 0 && right <= 0) {
			position_.y = t_left.y - body_->radius();
		}
		if (bottom > 0 && left <= 0 && right <= 0) {
			position_.y = b_right.y + body_->radius();
		}
		if (right > 0 && top <= 0 && bottom <= 0) {
			position_.x = t_right.x + body_->radius();
		}
		if (left > 0 && top <= 0 && bottom <= 0) {
			position_.x = b_left.x - body_->radius();
		}
