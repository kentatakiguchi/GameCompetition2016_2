...
void PlayerState_Idle::update(Actor & actor, float deltaTime){
	...
	if (InputMgr::GetInstance().IsMoving(KeyCode::W, KeyCode::S, KeyCode::D, KeyCode::A)) change(StateElement((unsigned int)Player_EnumState::MOVE));
	if (InputMgr::GetInstance().IsMoving(KeyCode::UP, KeyCode::DOWN, KeyCode::RIGHT, KeyCode::LEFT)) change(StateElement((unsigned int)Player_EnumState::MOVE));
