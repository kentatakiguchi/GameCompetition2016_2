...
public:
	...
	int countIntNumber(int drawNum, int baseNum, int posx, int posy, int NumTexType = 1, int maxSize = 3) const {
		int posCount = 0;
		...
		for (int i = 0; i < maxSize; i++) {
			...
			//if (i == dorC)DrawGraph(drawx - ResourceLoader::GetInstance().GetTextureSize(numberTexes_[NumTexType][drawNumberList[i]]).x*i, posy, ResourceLoader::GetInstance().getTextureID(numberTexes_[NumTexType][drawNumberList[i]]), TRUE);
			drawPosCount--;
		}
	...
	int countNumber(int dorC,int drawNum, int baseNum, int posx, int posy, int NumTexType = 1, int maxSize = 3) const{
		int posCount = 0;
		std::vector<int> drawNumberList;
	...
	void LoopPointNumber(int posx, int posy, int& useLoopType, int numPos, int NumTexType = 1) const {
		DrawGraph(posx + ResourceLoader::GetInstance().GetTextureSize(numberTexes_[NumTexType][NumberForLoop_[useLoopType]]).x*numPos, posy, ResourceLoader::GetInstance().getTextureID(numberTexes_[NumTexType][NumberForLoop_[useLoopType]]), TRUE);
		useLoopType++;
		if (useLoopType > 9) {
			useLoopType = 0;
		}
